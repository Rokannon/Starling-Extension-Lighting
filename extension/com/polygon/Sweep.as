package com.polygon
{
    public class Sweep
    {
        ...
        public function sweepPoints():void
        {
            ...
            {
                var point:Point = this.context.points[i];
        ...
        public function pointEvent(point:Point):Node
        {
            ...
            if (point.x <= (node.point.x + Constants.EPSILON))
            {
                fill(node);
            }
        ...
        public function edgeEventByPoints(ep:Point, eq:Point, triangle:Triangle, point:Point):void
        {
        ...
        public function fillAdvancingFront(n:Node):void
        {
            ...
            while (node.next != null)
            {
                ...
                if ((angle > Constants.PI_2) || (angle < -Constants.PI_2))
                {
                    break;
                }
                this.fill(node);
        ...
        {
            this.context.basin.left_node = ((Orientation.orient2d(node.point, node.next.point,
                                                                  node.next.next.point) == Orientation.CCW) ? node.next.next : node.next
                );
        ...
        {
            var height:Number = ( (this.context.basin.left_highest) ? this.context.basin.left_node.point.y - node.point.y : this.context.basin.right_node.point.y - node.point.y
                );
        ...
        public function fillRightConvexEdgeEvent(edge:Edge, node:Node):void
        {
            ...
            if (Orientation.orient2d(node.next.point, node.next.next.point,
        ...
        {
            // Next concave or convex?
            if (Orientation.orient2d(node.prev.point, node.prev.prev.point,
        ...
        public function flipEdgeEvent(ep:Point, eq:Point, t:Triangle, p:Point):void
        {
        ...
        public function flipScanEdgeEvent(ep:Point, eq:Point, flip_triangle:Triangle, t:Triangle, p:Point):void
        {
            var ot:Triangle = t.neighborAcross(p);
